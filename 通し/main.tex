...
%     \ifnum\value{\thesection}=0 S-\arabic{page} \else \arabic{\thesection}-\arabic{page} \fi
% } % フッターのページ番号,目次右"セクション番号―ページ番号"
% % \newcommand{\thesectionTOC}{\arabic{section}} % セクションタイトル，目次左""
% \renewcommand{\thesection}{\arabic{section}} % セクションタイトル，目次左""
...
\makeatother
\usepackage{hyperref}  % リンク用のパッケージ
% 目次のページ番号のカスタマイズ
\renewcommand{\thepage}{
    \ifnum\value{section}=0 S-\arabic{page} \else \arabic{section}-\arabic{page} \fi
}  % ページ番号に章番号を追加
...
\newcommand{\customsection}[1]{%
    ...
    \markboth{\thesection 章 \quad #1}{} % 目次に反映するためにセクション名と番号をセット
    ...
    \end{tcolorbox}%
    \addcontentsline{toc}{section}{\textbf\thesection 章 \, #1} % 目次に追加、ページ番号なし
    \clearpage
    \setcounter{page}{1} % ページ番号リセット
    \restoregeometry
}
...
\newcommand{\customsectionstar}[1]{%
    ...
    \end{tcolorbox}%
    \addtocontents{section}{\textbf #1}
    \addcontentsline{toc}{section}{\textbf{#1} } % 目次に追加
    \clearpage
    \setcounter{page}{1} % ページ番号リセット
    \restoregeometry
}
...
\newcommand{\sectionSP}[1]{%
    \vspace*{3em}
    \makebox[\textwidth]{% \textwidth 全体の幅を指定
        \fontsize{18pt}{18pt}{\selectfont\bfseries #1}
    }%
    \addcontentsline{toc}{section}{\textbf{#1}}
    % \phantomsection % 新しいリンク先を作成
    % \addtocontents{toc}{\protect\contentsline{section}{\protect\textbf #1}{}}
